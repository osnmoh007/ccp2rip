...
function CCPCalculator({ isDarkMode }) {
  ...
  const calculateCCP = () => {
    ...
    const rib = generateRip(ccpNumber, ripKey);

    setResults({ ccpKey, ripKey, rib });
    setCopiedStates({ rib: false, ripKey: false, ccpKey: false });
  };
  ...
  return (
    <div className="text-center mb-20">
      <h2 className={`text-6xl font-bold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>{t('calculator.title')}</h2>
      <p className={`text-xl mb-8 ${isDarkMode ? 'text-white' : 'text-gray-600'}`}>{t('calculator.description')}</p>
      ...
      {results && (
        <div className="mt-8 text-left max-w-md mx-auto">
          <h3 className={`text-2xl font-bold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>{t('results.title')}</h3>
          <div className={`p-4 rounded-md ${isDarkMode ? 'bg-gray-800' : 'bg-white shadow-md'}`}>
            <div className="mb-4">
              <p className="mb-1 flex items-center justify-between">
                <span>
                  <span className="font-bold">{t('results.rib')}:</span> {results.rib}
                </span>
                <button
                  onClick={() => copyToClipboard(results.rib, 'rib')}
              ...
              {copiedStates.rib && <p className="text-sm text-green-500 fade-out">{t('alerts.copiedToClipboard')}</p>}
            </div>
            <div className="mb-4">
              <p className="mb-1 flex items-center justify-between">
                <span>
